...
namespace AssemblyChain.Gh.Attributes
{
	internal class ComponentButton : GH_ComponentAttributes
	{
		...
		void SetMouseDown(bool value, GH_Canvas canvas, GH_CanvasMouseEvent e, bool invoke = true)
		{
			if (Owner.Locked || _mouseDown == value) return;
			if (value && e.Button != MouseButtons.Left) return;
			if (!_buttonBounds.Contains(e.CanvasLocation)) return;
			if (_mouseDown && !value && invoke) _action();
			_mouseDown = value;
			canvas.Invalidate();
		}
