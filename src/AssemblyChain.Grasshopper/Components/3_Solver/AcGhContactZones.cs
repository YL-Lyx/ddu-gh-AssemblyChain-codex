...
namespace AssemblyChain.Gh.Kernel
{
    public class AcGhContactZones : GH_Component
    {
        ...
        {
            const int maxDetailedContacts = 10;
            if (contactModel.ContactCount > maxDetailedContacts)
            {
                AddRuntimeMessage(GH_RuntimeMessageLevel.Remark,
                    $"Skipping detailed contact listing because there are more than {maxDetailedContacts} contacts.");
                return;
            }
