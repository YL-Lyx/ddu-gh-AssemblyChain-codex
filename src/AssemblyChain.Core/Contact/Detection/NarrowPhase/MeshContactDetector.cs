...
using AssemblyChain.Core.Toolkit.Geometry;
...
namespace AssemblyChain.Core.Contact.Detection.NarrowPhase
{
    ...
    public static partial class MeshContactDetector
    {
        ...
        private static bool ValidateInputs(Part partA, Part partB, PerformanceMonitor monitor, out string error)
        {
            ...
            if (!AssemblyChain.Core.Toolkit.Mesh.Preprocessing.MeshValidator.ValidateMeshForContactDetection(partA.Mesh, $"Part {partA.Name}", out var errorA))
            {
                error = $"MeshA validation failed: {errorA}";
                return false;
            }

            if (!AssemblyChain.Core.Toolkit.Mesh.Preprocessing.MeshValidator.ValidateMeshForContactDetection(partB.Mesh, $"Part {partB.Name}", out var errorB))
            {
